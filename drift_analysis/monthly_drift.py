...
def prod_model_drift_month(classifier,month):
    column_mapping = ColumnMapping()
    column_mapping.target = "target"
    column_mapping.prediction = "prediction"
    column_mapping.numerical_features = numerical_features
    ...
    X_cur, y_cur = current_data(month)
    cur_data = X_cur.copy()
    cur_data["target"] = y_cur
    cur_data["prediction"] = classifier.predict(X_cur)

    month_report = Report(metrics=[ClassificationPreset()], name=f"Monthly Model Drift Report - Month {month}", tags=[f"month_{month}", "drift"])
    month_report.run(
...
def target_drift(month):
    ...
    data_drift_report = Report(metrics=[DataDriftPreset()],name="Data Drift Report", tags=[f"month_{month}", "data drift"])
...
if __name__ == "__main__":
    ...
    PROJECT_NAME = "2008 Monthly Drift Analysis of weather Data"
    ...
    # Monthly reports
    for month in range(2, 13):  # From February (2) to December (12)
        model_drift_report_month = prod_model_drift_month(classifier, month)
        add_report(workspace, PROJECT_NAME, PROJECT_DESCRIPTION, model_drift_report_month)

        # Data drift report
        data_drift_report = target_drift(month)
